...
def get_config():
    ...
    check_results_queue_name = (import azure.functions as func
import logging
import lib.databricks_utils as dtb_utils
import lib.azkeyvault_utils as azkv_utils
import lib.azstorage_utils as azsg_utils
...
def get_sync_func_mappings():
    mappings = {
        "new-project-template": (
            "keyvault users",
            sync_keyvault_workspace_users_function,
        ),
        "azure-storage-blob": (
...
def send_exception_to_service_bus(exception_message):
    ...
    with servicebus.ServiceBusClient.from_connection_string(
        asb_connection_str, transport_type=servicebus.TransportType.AmqpOverWebsocket
    ) as client:
        with client.get_queue_sender(queue_name) as sender:
        ...
        if isinstance(dictionary[key], dict):
            ...
            if dictionary[key] and isinstance(dictionary[key][0], dict):
                res[key[0].upper() + key[1:]] = [
                    keys_upper(item) for item in dictionary[key]
                ]
        else:
            res[key[0].upper() + key[1:]] = dictionary[key]
    return res
