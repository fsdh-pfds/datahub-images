...
def assign_blob_reader_role(
    auth_client, subscription_id, scope, user_object_id, read_only
):
    if read_only:
        roleId = READER
    else:
    ...
    role_assignment_parameters = RoleAssignmentCreateParameters(
        role_definition_id="/subscriptions/{subscriptionId}/providers/Microsoft.Authorization/roleDefinitions/{roleId}".format(
            subscriptionId=subscription_id, roleId=roleId
        ),
...
def get_scope(subscription_id, resource_group_name, storage_account_name):
    return "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Storage/storageAccounts/{storageAccountName}".format(
            ...
            if user["Role"] == "Guest":
                if reader_role is None:
                    ...
                    assign_blob_reader_role(
                        client, subscription_id, scope, user_id, True
                    )
                ...
                if reader_role is None:
                    ...
                    assign_blob_reader_role(
                        client, subscription_id, scope, user_id, False
                    )
                elif reader_role:
                    print(f"assigning user {user_id} to access policies - read-write")
                    remove_existing_role(client, scope, user_id)
