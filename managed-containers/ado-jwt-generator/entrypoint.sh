...
[ -z "${IDENTITY_ENDPOINT:-}" ]      && { echo "Error: IDENTITY_ENDPOINT not set. Identity not exposed to container";      exit 1; }
[ -z "${IDENTITY_HEADER:-}" ]&& { echo "Error: IDENTITY_HEADER not set. Identity not exposed to container"; exit 1; }
[ -z "${MANAGED_IDENTITY_OBJECT_ID:-}" ]      && { echo "Error: MANAGED_IDENTITY_OBJECT_ID not set";      exit 1; }
