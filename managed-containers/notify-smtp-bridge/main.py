...
def is_private_ip(ip):
    ...
    try:
        ...
        return addr.is_private, addr.is_loopback or addr.is_link_local
    except ValueError:
        return False
...
async def _post_slack(subject: str, body: str, to: str):
    if not SLACK_WEBHOOK_URL:
        return
    payload = {"to": to, "subject": subject, "body": body[:1000]}

    # fire-and-forget to avoid blocking SMTP handler
    def _do():
...
class NotifyHandler:
    async def handle_DATA(self, server, session, envelope):
        ...
        # Gate by source IP (must be private/link-local/loopback)
        ...
        try:
            ...
            print(f"Malformed source IP {source_ip}, rejecting")
            return "550 Source IP unacceptable"
        ...
        reference = parsed.get("message_id")  # idempotency / reconciliation
        ...
        try:
            # fan-out: Slack signal (optional) and Notify send per recipient
            tasks = []
            for to in allowed:
                tasks.append(_post_slack(subject, body, to))
                tasks.append(_send_notify(to, subject, body, reference))
            results = await asyncio.gather(*tasks, return_exceptions=True)

            # Log failures but return SMTP 250 unless every Notify call failed
            failures = [r for r in results if isinstance(r, Exception)]
            if failures and len(failures) >= len(allowed):  # all Notify sends failed
                for f in failures:
